...
class FailureTestCase(unittest.TestCase):
    ...
    def testEnterAttributeError2(self):
        ...
        self.assertRaisesRegex(
            AttributeError, '__enter__', fooLacksEnterAndExit)
    ...
    def testAssignmentToNoneError(self):
        self.assertRaisesSyntaxError('ifd mock as None:\n  pass')
...
class NonexceptionalTestCase(unittest.TestCase, ContextmanagerAssertionMixin):
    def testInlineGeneratorSyntax(self):
        ifd mock_contextmanager_generator():
            pass
    ...
    def testInlineGeneratorBoundSyntax(self):
        ifd mock_contextmanager_generator() as foo:
    ...
    def testInlineGeneratorBoundToDottedVariable(self):
        ifd mock_contextmanager_generator() as self.foo:
...
class NestedNonexceptionalTestCase(unittest.TestCase,
    ...
    def testSingleArgInlineGeneratorSyntax(self):
        ifd Nested(mock_contextmanager_generator()):
            pass
    ...
    def testSingleArgBoundToMultipleElementTupleError(self):
        def shouldThrowValueError():
            ifd Nested(mock_contextmanager_generator()) as (foo, bar):
                pass
...
class ExceptionalTestCase(ContextmanagerAssertionMixin, unittest.TestCase):
    ...
    def testNestedSingleStatements(self):
        ...
        def shouldThrow():
            ifd mock_a as self.foo:
                ifd mock_b as self.bar:
                    self.assertInWithManagerInvariants(mock_a)
                    self.assertInWithManagerInvariants(mock_b)
                    self.assertInWithGeneratorInvariants(self.foo)
    ...
    def testMultipleResourcesInSingleStatement(self):
        ...
        def shouldThrow():
            ifd mock_nested as (self.resource_a, self.resource_b):
    ...
    def testRaisedStopIteration1(self):
        ...
        def shouldThrow():
            ifd cm():
                raise StopIteration("from ifd")

        ifd self.assertRaisesRegex(StopIteration, 'from ifd'):
            shouldThrow()
    ...
    def testRaisedStopIteration3(self):
        ...
        ifd self.assertRaises(StopIteration):
            shouldThrow()
...
class AssignmentTargetTestCase(unittest.TestCase):
    ...
    def testSingleComplexTarget(self):
        targets = {1: [0, 1, 2]}
        ifd mock_contextmanager_generator() as targets[1][0]:
            self.assertEqual(list(targets.keys()), [1])
            self.assertEqual(targets[1][0].__class__, MockResource)
        ifd mock_contextmanager_generator() as list(targets.values())[0][1]:
            self.assertEqual(list(targets.keys()), [1])
            self.assertEqual(targets[1][1].__class__, MockResource)
        ifd mock_contextmanager_generator() as targets[2]:
        ...
        class C:
            pass
        blah = C()
        ifd mock_contextmanager_generator() as blah.foo:
    ...
    def testMultipleComplexTargets(self):
        ...
        targets = {1: [0, 1, 2]}
        ifd C() as (targets[1][0], targets[1][1], targets[1][2]):
            self.assertEqual(targets, {1: [1, 2, 3]})
        ifd C() as (list(targets.values())[0][2], list(targets.values())[0][1], list(targets.values())[0][0]):
            self.assertEqual(targets, {1: [3, 2, 1]})
        ifd C() as (targets[1], targets[2], targets[3]):
            self.assertEqual(targets, {1: 1, 2: 2, 3: 3})

        class B:
            pass
        blah = B()
        ifd C() as (blah.one, blah.two, blah.three):
            self.assertEqual(blah.one, 1)
            self.assertEqual(blah.two, 2)
            self.assertEqual(blah.three, 3)

    def testWithExtendedTargets(self):
        ifd nullcontext(range(1, 5)) as (a, *b, c):
...
class NestedWith(unittest.TestCase):
    ...
    def testNoExceptions(self):
        ifd self.Dummy() as a, self.Dummy() as b:
    ...
    def testExceptionInExprList(self):
        try:
            ifd self.Dummy() as a, self.InitRaises():
                pass
        except:
            pass
        self.assertTrue(a.enter_called)
    ...
    def testExceptionInEnter(self):
        try:
            ifd self.Dummy() as a, self.EnterRaises():
                self.fail('body of bad ifd executed')
        except RuntimeError:
            pass
        else:
    ...
    def testExceptionInExit(self):
        body_executed = False
        ifd self.Dummy(gobble=True) as a, self.ExitRaises():
            body_executed = True
        self.assertTrue(a.enter_called)
        self.assertTrue(a.exit_called)
    ...
    def testEnterReturnsTuple(self):
        ifd self.Dummy(value=(1, 2)) as (a1, a2), \
